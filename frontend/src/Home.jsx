...
function Home() {
  ...
  // Modal state (front page Add Todo)
  ...
  const prevCompletionMap = useRef(new Map()); // track previous completion to detect 100%
  ...
  // Map todos by entry for quick lookup
  ...
  // Compute completion per entry and trigger high-five if newly 100%
  useEffect(() => {
  ...
  const addTodo = async (e) => {
    ...
    try {
      ...
      // Adjust to your API. If you have POST /entries/:id/todos, use that.
      // Fallback shown: POST /todos with { entry_id, text }
      ...
      if (!res.ok) {
        // try generic /todos
        res = await fetch(`http://localhost:8000/todos`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ entry_id: activeEntry.entry_id, text: newTodoText.trim() })
        });
      }
